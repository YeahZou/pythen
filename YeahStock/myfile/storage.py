...
路径格式：
data/sina/stock/day/202005/31/12345_中国平安.txt
data/eastmoney/stock/week/202005/31/12345_中国平安.txt
"""
d = datetime.date.today()
YEAR, MONTH, DAY = str(d.year), str(d.month).zfill(2), str(d.day).zfill(2)
def save_to_file(datasource, stype, dtype, symbol, name, data):
    try:
        path = os.path.join(ROOT_PATH, datasource, stype, dtype, YEAR + MONTH, DAY)
        filename = symbol + '_' + name + '.txt'

        if (not os.path.exists(path)):
            os.makedirs(path)
    
        fhandler = open(os.path.join(path, filename), 'w', encoding='utf-8')
